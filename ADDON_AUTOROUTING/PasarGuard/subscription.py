# Измененный файл /code/app/operation/subscription.py
...
_ROUTING_URLS = {
    "default": "https://raw.githubusercontent.com/LibreGate/routing/main/HAPP/DEFAULT.DEEPLINK",
    "jsonsub": "https://raw.githubusercontent.com/LibreGate/routing/main/HAPP/JSONSUB.DEEPLINK",
    "whitelist": "https://raw.githubusercontent.com/LibreGate/routing/main/HAPP/WHITELIST.DEEPLINK",
}


class _RoutingResolver:
    def __init__(self, default_source: str = "default"):
        self._lock = threading.Lock()
        self._value = ""
        self._fetched_at = 0.0
        self._last_fail = 0.0
        self._source = os.environ.get("ROUTING_SOURCE", default_source).strip().lower()
        self._custom = os.environ.get("ROUTING_CUSTOM", "").strip()

    def _fetch_url(self, url: str) -> str:
        request = Request(
            url,
            headers={
                "User-Agent": "PasarGuard-Routing-Resolver/1.0",
            },
        )
        with urlopen(request, timeout=4) as response:
            content = response.read()
            charset = response.headers.get_content_charset() or "utf-8"
            return content.decode(charset, errors="replace").strip()
    ...
    def get(self) -> str:
        ...
        with self._lock:
            ...
            try:
                self._value = self._fetch_url(url)
...
routing_resolver = _RoutingResolver()
...
class SubscriptionOperation(BaseOperation):
    ...
    @staticmethod
    async def detect_client_type(
        user_agent: str, rules: list[SubRule]
    ) -> ConfigFormat | None:
    ...
    def create_response_headers(
        ...
        formatted_title = SubscriptionOperation._format_profile_title(
            user, format_variables, sub_settings
        )
        ...
        headers = {
            ...
            "subscription-userinfo": "; ".join(
                f"{key}={val}" for key, val in user_info.items()
            ),
            "announce": encode_title(sub_settings.announce),
            "announce-url": sub_settings.announce_url,
        }

        routing_value = routing_resolver.get()
        if routing_value:
            headers["routing"] = routing_value
            headers["routing-enable"] = "true"
    ...
    def _format_rule_response_headers(
        cls,
        rule: SubRule | None,
        format_variables: dict[str, str | int | float],
    ) -> dict[str, str]:
        ...
        for raw_name, raw_value in rule.response_headers.items():
            ...
            formatted_value = cls._stringify_rule_header_value(
                raw_value, format_variables
            )
    ...
    @staticmethod
    def _stringify_rule_header_value(
        value: Any, format_variables: dict[str, str | int | float]
    ) -> str:
    ...
    @staticmethod
    def create_info_response_headers(
        user: UsersResponseWithInbounds, sub_settings: SubSettings
    ) -> dict:
                ...
                render_template(
                    template,
                    {
                        ...
                        "apps": self._make_apps_import_urls(
                            sub_settings.applications, format_variables
                        ),
                    },
                )
            )
    ...
    async def user_subscription_with_client_type(
        ...
        if client_type == ConfigFormat.block or not getattr(
            sub_settings.manual_sub_request, client_type
        ):
            await self.raise_error(message="Client not supported", code=406)
    ...
    def _make_apps_import_urls(
        self, applications: list[Application], format_variables: dict
    ) -> list[Application]:
